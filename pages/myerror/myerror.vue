	...
	<view class="content">
		<view class="item" v-for="(item, index) in examRecodDatasList" :key="index">
			<view class="weekTitle">
				...
				<text class="num" style="color: #ff3d5d;">错{{item.count}}次</text>
			</view>
	...
	export default {
		...
		methods: {
			...
			fetchexamRecodDatasList: function() {
				...
				var baseUrl = that.$constants.EXAM_MYERRORS + "?page=" + page;
				that.$request
					.get({
						url: baseUrl,
						loadingTip: '加载中...',
					...
					.then(
						res => { //success
							...
							for (var i = 0; i < res.myerror.length; i++) {
								var item = res.myerror[i];
