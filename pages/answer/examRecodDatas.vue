	...
	<view class="content">
		<view class="item" v-for="(item, index) in examRecodDatasList" :key="index">
			<view class="weekTitle">
				...
				<text class="num" style="color: #99cc33;" v-if="item.isright=='1'">回答正确</text>
				<text class="num" style="color: #ff3d5d;" v-if="item.isright=='0'">回答错误</text>
			</view>
			<!--状态一：用户确认答案，答案正确-->
			<view class="details" v-if="item.isright=='1'">
				<view class="title">{{index+1}}.{{item.question}}</view>
				<image :src="item.thumb" mode="aspectFit" v-if="item.showTitle"></image>
				<view class="option" v-for="(sitem, sindex) in item.items" :key="sindex">
			...
			<!--状态二：用户确认答案，答案错误-->
			<view class="details" v-if="item.isright=='0'">
				<view class="title">{{index+1}}.{{item.question}}</view>
				<image :src="item.thumb" mode="aspectFit" v-if="item.showTitle"></image>
			...
			<view class="analysis">
				...
				<view class="answer">
					<view  v-if="item.type != '1'"><text style="color: #99cc33;">{{item.q_answer}}</text></view>
					<view  v-else><text style="color: #99cc33;">{{item.q_answer == '1'?'A':'B'}}</text></view>
				</view>
				<view class="weekTitle2">
...
<script>
	...
	export default {
		...
		onLoad(options) {
			var that = this;
			if (options.hasOwnProperty('record_id')) {
				that.id = options.record_id;
		...
		methods: {
			...
			fetchexamRecodDatasList: function() {
				var that = this;
				if (page == 1) {
					var examRecodDatasList = [];
				} else {
					var examRecodDatasList = that.examRecodDatasList;
				}
				var baseUrl = that.$constants.EXAM_EXAMRECODDATAS + "?record_id=" + that.id + "&page=" + page;
				that.$request
					.get({
						url: baseUrl,
						loadingTip: '加载中...',
					...
					.then(
						res => { //success
							...
							for (var i = 0; i < res.examRecodDatasList.length; i++) {
								var item = res.examRecodDatasList[i];
								if (item.hasOwnProperty('items')) {
									if (!that.$utils.isEmpty(item.items)) {
										var options = [];
										item.items.forEach((sitem, sindex) => {
											if (sitem['item_value'] == item.answer) {
												sitem.checked = true;
											} else {
												sitem.checked = false;
											}
								...
								examRecodDatasList.push(item);
							}
							console.log("examRecodDatasList", examRecodDatasList)
							that.examRecodDatasList = examRecodDatasList;
						},
						res => { //fail
